...
class OrderService {
  async validateOrder(cartId, couponCodes) {
    ...
    const { total, discountApplied, couponCodeString } =
      await this.calculateTotalWithCoupons(cart, couponCodes);
  ...
  async verifyStockAvailability(cartItems) {
    for (const item of cartItems) {
      if (item.quantity > item.product_id.stock) {
        throw new Error(
          `Not enough stock for product ID: ${item.product_id.title}`
        );
      }
    }
  }
  ...
  async calculateTotalWithCoupons(cart, couponsCodes) {
    ...
    if (!cartItems.length)
      return { total: 0, discountApplied: 0, couponCodes: [] };
    ...
    for (const code of couponsCodes) {
      ...
      for (const item of cartItems) {
        if (
          ...
          item.product_id.category.toString() === coupon.category_id.toString()
        ) {
    ...
    return {
      total,
      discountApplied,
      couponCodeString: appliedCoupons.join(','),
    };
  }
